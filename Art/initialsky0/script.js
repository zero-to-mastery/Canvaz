class RainbowCircle {
   ...
   calcGradCoord(circRad, degRotate, clockwise) {
      ...
      if(clockwise) {
         startCoordX = this.origin[0]-(radius*Math.cos(degRotate*(Math.PI / 180)));
         endCoordX = this.origin[0]+(radius*Math.cos(degRotate*(Math.PI / 180)));
      } else {
         startCoordX = this.origin[0]+(radius*Math.cos(degRotate*(Math.PI / 180)));
         endCoordX = this.origin[0]-(radius*Math.cos(degRotate*(Math.PI / 180)));
      }

      startCoordY = this.origin[1]-(radius*Math.sin(degRotate*(Math.PI / 180)));
